// 流动线
export const _getFlowLineShader = (options) => {
...
// 动态线
export const _getDynamicLineShader = (options) => {
  if (options && options.get) {
...
// 动态泛光线
export const _getDynamicLightLineShader = (options) => {
  if (options && options.get) {
    return 'czm_material czm_getMaterial(czm_materialInput materialInput)\n\
    {\n\
        czm_material material = czm_getDefaultMaterial(materialInput);\n\
 ...
 * @param {*} options
 */
export const _getDirectionWallShader = (options) => {
  if (options && options.get) {
    let materail =
      'czm_material czm_getMaterial(czm_materialInput materialInput)\n\
        {\n\
...
export const _getCircleFadeShader = (options) => {
  if (options && options.get) {
    return `czm_material czm_getMaterial(czm_materialInput materialInput)\n
        {\n
            czm_material material = czm_getDefaultMaterial(materialInput);\n
...
// 波动圆
export const _getDynamicCircleShader = (options) => {
  if (options && options.get) {
    return 'uniform vec4 color;\n\
    uniform float duration;\n\
    uniform float count;\n\
...
// 雷达扫描
export const _getRadarScanShader = (options) => {
  if (options && options.get) {
    return (
...
// 圆形扫描
export const _getCircleScanShader = (options) => {
  if (options && options.get) {
    return (
      'uniform sampler2D colorTexture;\n\
    uniform sampler2D depthTexture;\n\
